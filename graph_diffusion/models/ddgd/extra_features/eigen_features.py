...
def get_eigenvalues_features(
    ...
    """
    # TODO: the following code is not compatible with JAX's jit
    # to_extend = (eigenvalues.shape[0], np.max(n_connected_components) + k - n)
    ...
    eigenvalues = np.hstack((eigenvalues, 2 * np.ones((bs, to_extend))))
    # basic = np.arange(k, dtype=np.int32)
    # new_eigenvalues = np.where(
    #     (basic < len(eigenvalues))[..., None], eigenvalues[basic], 2
    # )
