...
from ....shared.graph.graph_distribution import GraphDistribution
...
def node_cycle_features(graph: GraphDistribution) -> tuple[Array, Array]:
    adj_matrix = graph.edges[..., 1:].sum(axis=-1).astype(float)

    x_cycles, y_cycles = k_cycles(adj_matrix=adj_matrix)  # (bs, n_cycles)
    x_cycles = x_cycles.astype(adj_matrix.dtype) * graph.nodes_mask[..., None]
