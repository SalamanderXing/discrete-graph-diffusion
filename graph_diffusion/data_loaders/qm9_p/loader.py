...
import os
import h5py
from .main import QM9DataModule
...
def create_graph(nodes, edges, edges_counts, nodes_counts, train=False):
    ...
    if train:
        # Assuming node_matrix is of shape (batch, n, embedding_node)
        # and edge_matrix is of shape (batch, n, n, embedding_edge)

        batch_size, n, _ = nodes.shape

        # Generate a random permutation for each graph in the batch
        permutations = np.array([np.random.permutation(n) for _ in range(batch_size)])

        # Reorder nodes
        nodes = np.take_along_axis(nodes, permutations[:, :, None], axis=1)
        # Reorder edges
        edges = np.take_along_axis(
            np.take_along_axis(edges, permutations[:, :, None, None], axis=1),
...
def load_data(*, save_path: str, train_size: float = 0.8, batch_size: int):
    ...
    all_edges_counts = []
    all_node_maks = []
    for key, _ in data_module.dataloaders.items():
        dataset = getattr(data_module, f"{key}_dataloader")()
        items = len(dataset)
        ...
        max_n_atom = val.x.shape[1]
        num_edge_features = val.edge_attr.shape[-1] + 1
        ...
        all_nodes_counts.append(num_nodes_list)
        all_edges_counts.append(edges_counts)
        all_node_maks.append(node_masks)
    ...
    num_nodes_list = np.concatenate(all_nodes_counts, axis=0)
    edges_counts = np.concatenate(all_edges_counts, axis=0)
    node_masks = np.concatenate(all_node_maks, axis=0)
    items = len(nodes)
