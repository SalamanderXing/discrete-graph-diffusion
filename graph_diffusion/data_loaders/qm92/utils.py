...
# import torch
from ...shared.jax_geometric.utils import (
    to_dense_adj,
    to_dense_batch,
)
...
#     first_elt = E[:, :, :, 0]
#     first_elt = first_elt.at[no_edge].set(1)
#     E = E.at[:, :, :, 0].set(first_elt)
#     # diag = (
#     #     torch.eye(E.shape[1], dtype=torch.bool).unsqueeze(0).expand(E.shape[0], -1, -1)
#     # )
#     diag = np.tile(np.eye(E.shape[1], dtype=bool)[None], (E.shape[0], 1, 1))
#     E = E.at[diag].set(0)
#     return E
from jax import jit
...
def encode_no_edge(E):
    ...
    E = np.where(np.expand_dims(no_edge, axis=-1), first_elt[:, :, :, None], E)
    diag = np.tile(np.eye(E.shape[1], dtype=bool)[None], (E.shape[0], 1, 1))
    E = np.where(np.expand_dims(diag, axis=-1), 0, E)
    return E
...
#
#         if collapse:
#             self.X = torcdwih.argmax(self.X, dim=-1)
...
class PlaceHolder:
    ...
    def type_as(self, x):
        """Changes the dtype of X, E, y to match x."""
    ...
    def mask(self, node_mask, collapse=False):
        x_mask = np.expand_dims(node_mask, axis=-1)  # bs, n, 1
        e_mask1 = np.expand_dims(x_mask, axis=2)  # bs, n, 1, 1
        e_mask2 = np.expand_dims(x_mask, axis=1)  # bs, 1, n, 1
