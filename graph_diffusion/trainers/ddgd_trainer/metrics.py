...
class SamplingMolecularMetrics:
    def __init__(self, dataset_infos, train_smiles):
        super().__init__()
        self.train_smiles = train_smiles
        self.dataset_info = dataset_infos

    def __call__(self, molecules: gd.GraphDistribution):
        molecule_list = []
        n_nodes = molecules.nodes_counts
        nodes = np.where(molecules.nodes_mask, molecules.nodes.argmax(-1), -1)
        edges = np.where(molecules.edges_mask, molecules.edges.argmax(-1), -1)
        for i in range(molecules.batch_size):
            n = n_nodes[i]
            atom_types = nodes[i, :n]
            edge_types = edges[i, :n, :n]
            molecule_list.append([atom_types, edge_types])
        return compute_molecular_metrics(
...
def build_molecule(atom_types, edge_types, atom_decoder, verbose=False):
    ...
    all_bonds = np.nonzero(edge_types)
    for from_atom, to_atom in zip(all_bonds[0], all_bonds[1]):
        from_atom = from_atom.item()
        to_atom = to_atom.item()
        if from_atom != to_atom:
            mol.AddBond(
                from_atom,
                to_atom,
                bond_dict[edge_types[from_atom, to_atom].item()],
            )
    return mol
    ...
    for from_atom, to_atom in zip(all_bonds[0], all_bonds[1]):
        ...
        if from_atom != to_atom:
            mol.AddBond(
                from_atom,
                to_atom,
                bond_dict[edge_types[from_atom, to_atom]],
            )
