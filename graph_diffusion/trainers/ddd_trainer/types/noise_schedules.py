...
class NoiseSchedule(nn.Module):
    ...
    @typed
    def __call__(self, t: Int[Array, "batch"]):
        raise NotImplementedError

    def __getitem__(self, t: Int[Array, "batch"]):
        t = jnp.where(t < 0, self.steps + t, t)
        betas = self.__call__(t)
        # betas = cosine_beta_schedule_discrete(diffusion_steps, 10000.0)
        # betas will be a linear schedule
        betas = np.linspace(0.2, 0.8, self.steps)
