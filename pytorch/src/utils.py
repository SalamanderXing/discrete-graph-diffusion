...
def softmax_kl_div(tensor1, tensor2, reduction="mean"):
    # Subtract maximum value for numerical stability
    tensor1_max, tensor2_max = (
        tensor1.max(dim=-1, keepdim=True).values,
        tensor2.max(dim=-1, keepdim=True).values,
    )
    tensor1_stable, tensor2_stable = tensor1 - tensor1_max, tensor2 - tensor2_max

    # Compute log-sum-exp for both tensors
    log_sum_exp1 = torch.logsumexp(tensor1_stable, dim=-1, keepdim=True)
    log_sum_exp2 = torch.logsumexp(tensor2_stable, dim=-1, keepdim=True)
    ...
    proportional_softmax1 = tensor1_diff.exp()
...
"""
def test_kl_divergence_optimized():
    # Generate random input tensors
    batch_size = 8
    num_classes = 10
    tensor1 = torch.randn(batch_size, num_classes).log_softmax(dim=-1)
    tensor2 = torch.randn(batch_size, num_classes).log_softmax(dim=-1)

    # Calculate KL divergence using PyTorch's built-in kl_div function
    kl_div_builtin = F.kl_div(tensor2, tensor1.exp(), reduction="none").mean(dim=-1)

    # Calculate KL divergence using the custom kl_divergence_optimized function
    kl_div_custom = softmax_kl_div(tensor1, tensor2, reduction="batchmean")

    # Compare the results
    assert torch.allclose(
        kl_div_builtin, kl_div_custom, atol=1e-6
    ), f"Results do not match {kl_div_builtin=} vs {kl_div_custom=}"

    print("Results match! Both methods produced the same KL divergence values.")
"""

import time


def test_kl_divergence_optimized(num_tests=1000):
    ...
    for _ in range(num_tests):
        ...
        kl_div_builtin = F.kl_div(tensor2, tensor1.exp(), reduction="none")
        end_time_builtin = time.perf_counter()

        # Calculate KL divergence using the custom kl_divergence_optimized function
        start_time_custom = time.perf_counter()
        kl_div_custom = softmax_kl_div(tensor1, tensor2, reduction="none")
        ...
        total_builtin_time += end_time_builtin - start_time_builtin
        total_custom_time += end_time_custom - start_time_custom

    print("Results match! Both methods produced the same KL divergence values.")
    print(
        f"Average time for built-in function: {total_builtin_time / num_tests:.6f} seconds"
    )
    print(
        f"Average time for custom function: {total_custom_time / num_tests:.6f} seconds"
    )


if __name__ == "__main__":
    test_kl_divergence_optimized()
