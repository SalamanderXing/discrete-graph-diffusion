...
class GAT(nn.Module):
    ...
    num_features_per_layer: tuple[int, ...]
    ...
    def setup(
        ...
        num_heads_per_layer = (
            1,
        ) + self.num_heads_per_layer  # trick - so that I can nicely create GAT layers below
...
class GATLayer(nn.Module):
    ...
    src_nodes_dim: int = 0  # position of source nodes in edge index
    trg_nodes_dim: int = 1  # position of target nodes in edge index

    dropout_prob = 0.6
    # These may change in the inductive setting - leaving it like this for now (not future proof)
    nodes_dim: int = 0  # node dimension (axis is maybe a more familiar term nodes_dim is the position of "N" in tensor)
    head_dim: int = 1  # attention head dim
    add_skip_connection: bool = True
    bias: bool = True
    ...
    def __init__(
        ...
        self.linear_proj = nn.Dense(
            self.num_of_heads * self.num_out_features, bias=False
        )
